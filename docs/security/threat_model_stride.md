...
| DB-T1 | **Tampering** | Audit log tampering after the fact. | Request handlers only ever INSERT into `audit_log` and `pii_detections` — there is no UPDATE or DELETE path from the API surface. The retention job (`retentionJob.ts`) is the single place where `audit_log` rows are ever mutated, and only to anonymize (null `user_id` + `ip_address`) past the maximum audit window; rows themselves are preserved. Recommended hardening: split DB roles so the application user has INSERT-only grants on these tables and a separate `retention_writer` role runs the anonymization. |
